  ...
  for (let item of historyItems)
    if (item.title != 'History') {
      ...
      pageName.innerHTML = `${shorten(
        item.title,
        2
      )}  <br> <span class='timeVis'>${getDate(item.lastVisitTime)}</span>`
...
function createFirstInfo (str, dataset) {
  ...
  var color = d3.scale
    ...
    .range([
      ...
      '#267257',
      '#AA6A39'
    ]) //['#0b1307', '#203815', '#365d22', '#7bc158', '#98cf7d', '#d3eac7']);
  var arc = d3.svg
    .arc()
...
chrome.history.search({ text: '', maxResults: 0 }, function (historyItems) {
  ...
  for (var i = 0; i < historyItems.length; i++) {
    ...
    chrome.history.getVisits({ url: historyItems[i].url }, function (
      visitItems
    ) {
      for (var j = 0; j < visitItems.length; j++) {
        ...
        if (mapTransition.get(visitItems[j].transition)) {
          mapTransition.set(
            visitItems[j].transition,
            mapTransition.get(visitItems[j].transition) + 1
          )
        } else {
...
function getUniqueAlph (str) {
  //console.log(str);
  if (str.trim().length == 0) return 'A'
  let arr = str.split('.')
  if (arr.length >= 2) return arr[arr.length - 2].charAt(0).toUpperCase()
  return ''
}
